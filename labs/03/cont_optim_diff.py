...
DIFF_F = 0  # Differential F param
DIFF_C = 0  # Differential C param
...
class DiffMutation:
    ...
    def __call__(self, ind):
        ...
        donor = partners[1] + DIFF_F * \
            (np.sum(partners[2::2]) - np.sum(partners[3::2]))
        offspring = [donor[i] if np.random.rand() < DIFF_C else ind[i]
