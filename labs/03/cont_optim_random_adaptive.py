...
DIMENSION = 15  # dimension of the problems
POP_SIZE = 100  # population size
MAX_GEN = 500  # maximum number of generations
CX_PROB = 0.0  # crossover probability
IA_CX_PROB = 0.0  # intelligent arithmetic crossover probability
R_CX_PROB = 0.8  # intelligent arithmetic crossover probability
MUT_PROB = 0.2  # mutation probability
MUT_STEP = 0.5  # size of the mutation steps
REPEATS = 10  # number of runs of algorithm (should be at least 10)
ADAPTIVE_VAR = 0.5  # adaptive variance
OUT_DIR = 'continuous'  # output directory for logs
# the ID of this experiment (used to create log names)
EXP_ID = 'random_adaptive'
MIN_VAL = -5
MAX_VAL = 5
...
def create_ind(ind_len):
    return np.random.uniform(MIN_VAL, MAX_VAL, size=(ind_len,))

# creates the individual


def create_ind_adaptive(ind_len):
    return np.append(np.random.uniform(-5, 5, size=(ind_len,)), np.ones((ind_len,)))
...
def intelligent_arithmetic_cross(p1, p2, fitness):
    w1 = fitness(p1).fitness
    w2 = fitness(p2).fitness
    S = w1 + w2
    w1 /= S
    w2 = 1-w1

    return w1*p1+w2*p2, w2*p1+w1*p2
...
class AdaptiveMutation:
    def __call__(self, ind):
        return ind + np.random.normal(scale=np.append(
            ind[ind.shape[0]:], np.ones(ind.shape[0]) * ADAPTIVE_VAR))
...
if __name__ == '__main__':
    ...
    for fit_gen, fit_name in zip(fit_generators, fit_names):
        ...
        iaxover = functools.partial(
            crossover, cross=functools.partial(intelligent_arithmetic_cross, fitness=fit), cx_prob=IA_CX_PROB)
        ...
        for run in range(REPEATS):
            ...
            pop = evolutionary_algorithm(pop, MAX_GEN, fit, [
                                         rxover, mut], tournament_selection, mutate_ind, map_fn=map, log=log)
